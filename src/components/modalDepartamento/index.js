...
export default function ModalDepartamento({ showModal, setShowModal }) {
            ...
            .then((data) => {
              createCiudad(body)
                .then(() => {
                  ...
                  Swal.fire(
                    "¡Correcto!",
                    "El país se ha agregado con éxito",
                    "success"
                  );
                  setShowModal(!showModal);
                })
                .catch((err) => {
                  Swal.fire(
                    "Uops!",
                    "Hubo un error al momento de registrar el pais. intente de nuevo. Si el problema persiste comuniquese con su jefe directo ó con el área de sistemas para darle una oportuna y rápido solución.",
                    "warning"
                  );
                  setShowModal(!showModal);
                });
            ...
            .then((data) => {
              updateCiudad(info.codigo, body)
                ...
                .catch((err) => {
                  Swal.fire(
                    "Uops!",
                    "Hubo un error al momento de editar el pais. intente de nuevo. Si el problema persiste comuniquese con su jefe directo ó con el área de sistemas para darle una oportuna y rápido solución.",
                    "warning"
                  );
                  setShowModal(!showModal);
                });
          ...
          .then(() => {
            deleteCiudad(info.codigo)
              ...
              .catch((err) => {
                Swal.fire(
                  "Uops!",
                  "Hubo un error al momento de eliminar el pais. intente de nuevo. Si el problema persiste comuniquese con su jefe directo ó con el área de sistemas para darle una oportuna y rápido solución.",
                  "warning"
                );
                setShowModal(!showModal);
              });
      ...
      >
        ...
        <Modal.Body className="p-2">
          <div className="m-2 h-100">
            <form
              onSubmit={
                editar ? handleUpdateDepartamento : handleCreateNewDepartamento
              }
            >
              ...
              {vacio ? (
                <span className="d-flex w-100 text-align-center text-danger me-3">
                  Ingrese un código para hacer la busqueda
                </span>
              ) : (
                ""
              )}
              {notFound ? (
                <span className="d-flex w-100 justify-content-center text-align-center text-danger">
                  Esta clasificación no se encuentra en nuestra base de datos
                </span>
              ) : (
                ""
              )}
          ...
          >
            <div className="d-flex justify-content-center gap-0 mt-2 ">
              ...
              <button
                ...
                onSubmit={
                  editar
                    ? handleUpdateDepartamento
                    : handleCreateNewDepartamento
                }
                className="me-3"
                type="submit"
              >
