...
import { createContrato , updateContrato , findOne , deleteContrato } from "../../services/contratoService";
...
export default function ModalContrato({
  ...
  useEffect(()=>{
    getAllContratos().then((data) => (setContratos(data), setSuggestions(data)));
  },[showModal])
        ...
        if(result.isConfirmed){
          ...
          createContrato(body)
            .then((data) => {
              ...
              Swal.fire(
                '¡Correcto!', 'El tipo de contrato se ha agregado con éxito', 'success'
              )
  ...
  const findByDescrip = (e) => {
    const { value } = e.target;
    setEditar(false)
    const item = contratos.find((elem)=> elem.description.toLowerCase() === value.toLowerCase());
    if(item){
  ...
  return (
    ...
    <Modal show={showModal} style={{ fontSize: 18, userSelect:'none' }} centered>
      ...
      <Modal.Body className="p-2">
        <div className="m-2 h-100">
          <form onSubmit={itemSeleccionado ? handleUpdateContrato : handleCreateNewContrato}>
            ...
            {notFound ?
              <span className="d-flex w-100 text-align-center text-danger me-3">Este tipo de contrato no se encuentra en nuestra base de datos</span>
              :''
            }
          </form>
        </div>
      ...
      <div className="d-flex justify-content-center gap-0 mt-2 ">
              {!editar &&
                <div>
                  <button style={{backgroundColor:'#008F39'}} onClick={(e)=> itemSeleccionado ? handleEditar(e) : handleCreateNewContrato(e)} className="me-3" type="submit">{itemSeleccionado ? "Actualizar" : "Crear"}</button>
                </div>
              }
              ...
              {editar &&
                <div>
                  <button style={{backgroundColor:'#008F39'}} onClick={(e)=>handleUpdateContrato(e)} className="me-3" type="submit">Guardar</button>
                  <button onClick={(e)=>handleCancelEditar(e)} className="me-3" type="submit">Descartar</button>
                </div>
              }
            </div>
            </form>
