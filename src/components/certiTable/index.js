...
export default function CertiTable({ terceros, loading }) {
  ...
  const suma = terceros.reduce((acumular, item) => {
    const numeroNormal = parseFloat(
      item.base.replace(/[,.]/g, "") /* .replace(/./g, '') */
    );
  ...
  const credit = terceros.reduce((acumular, item) => {
    const numeroNormal = parseInt(
      item.sumaCredito.replace(/[,.]/g, "") /* .replace(/./g, '') */
    );
    return acumular + numeroNormal;
  }, 0);
  ...
  const totalRow = {
    tipoCertificado: "TOTAL",
    cuenta: "",
    concepto: "",
    sumaDebito: "",
    sumaCredito: totalCredito,
    sumaMovimiento: "",
    sumaValorBase: totalBase,
    ciudadIca: "",
    tasa: "",
  };
  ...
  return (
    <div
      className="wrapper justify-content-center d-flex flex-column rounded w-100 h-100"
      style={{ userSelect: "none", fontSize: 20 }}
    >
      <div
        className="login-wrapper justify-content-center shadow border border-2 rounded-4 "
        style={{ width: 1000, maxHeight: 350, backgroundColor: "white" }}
      >
        <div className="d-flex w-100 justify-content-start mb-2">
          <Button
            onClick={(e) => navigate("/consultar/certificado")}
            variant="contained"
          >
        ...
        <div className="d-flex w-100 justify-content-center mt-2">
          <Button
            ...
            color="success"
            className="me-4"
            onClick={handlerFiltroRiva}
          >
            Certificado ReteIva
          </Button>
          <Button
            onClick={handlerFiltroRica}
            variant="contained"
            color="info"
            className="me-4"
          >
          ...
          <Button onClick={handlerFiltroRfte} variant="contained" color="error">
            Certificado retefuente
          </Button>
        </div>
      </div>
    </div>
