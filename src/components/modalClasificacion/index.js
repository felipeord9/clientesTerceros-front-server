...
export default function ModalClasificacion({ showModal, setShowModal }) {
    ...
    Swal.fire({
      ...
      .then((result) => {
        if (result.isConfirmed) {
          createClasificacion(info).then((data) => {
            setShowModal(!showModal);
            Swal.fire(
              "¡Correcto!",
              "La clasificación se ha agregado con éxito",
              "success"
            );
          });
  ...
  const handleUpdateAgency = (e) => {
    ...
    Swal.fire({
      ...
      .then((result) => {
        if (result.isConfirmed) {
          updateClasificacion(info.id, info).then((data) => {
            setShowModal(!showModal);
            Swal.fire({
              title: "¡Correcto!",
      ...
      >
        ...
        <Modal.Body className="p-2">
          <div className="m-2 h-100">
            <form
              onSubmit={
                editar ? handleUpdateAgency : handleCreateNewClasificacion
              }
            >
