...
export default function ModalCargos({ showModal, setShowModal }) {
    ...
    Swal.fire({
      ...
      .then((result) => {
        if (result.isConfirmed) {
          ...
          createCargo(body).then((data) => {
            setShowModal(!showModal);
            Swal.fire(
              "¡Correcto!",
              "El cargo se ha agregado con éxito",
              "success"
            );
          });
  ...
  const handleUpdateCargo = (e) => {
    ...
    Swal.fire({
      ...
      .then((result) => {
        if (result.isConfirmed) {
          updateCargo(info.id, info).then((data) => {
            setShowModal(!showModal);
            Swal.fire({
              title: "¡Correcto!",
            ...
            <form onSubmit={editar ? handleUpdateCargo : handleCreateNewCargo}>
              <div className="d-flex w-100 flex-row">
                <div className="d-flex flex-column me-2 w-100">
                  ...
                  <input
                    id="id"
                    type="number"
                    value={itemSeleccionado ? itemSeleccionado.id : info?.id}
                    className="form-control form-control-sm"
                    onChange={(e) => (handleChange(e), findById(e))}
                    autoComplete="off"
                    required
                  />
                </div>
              ...
              {notFound ? (
                <span className="d-flex w-100 text-align-center text-danger me-3">
                  Este cargo no se encuentra en nuestra base de datos
                </span>
              ) : (
                ""
              )}
                ...
                <button
                  ...
                  onClick={(e) =>
                    itemSeleccionado
                      ...
                      : handleCreateNewCargo(e)
                  }
                  className="me-3"
                  type="submit"
                >
                  {itemSeleccionado ? "Editar" : "Crear"}
                </button>
              )}
              {editar ? (
                <button
