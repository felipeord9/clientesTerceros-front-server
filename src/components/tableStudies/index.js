...
function Tablestudies({ formData, setFormData, currentStudy, setCurrentStudy, editIndex, setEditIndex, list, setList }) {
  ...
  return (
    <div className="table-responsive mt-2 mb-3 rounded" style={{fontSize:13}}>
      <table className="table table-bordered table-hover align-middle text-center m-0 caption-top">
        ...
        <thead >
          <tr >
            ...
            <th style={{ width: 120 , backgroundColor:'whitesmoke'}}>Estado</th>
            <th style={{backgroundColor:'whitesmoke'}}>Título</th>
            <th style={{backgroundColor:'whitesmoke'}}>Establecimiento</th>
            <th style={{ width: 80 , backgroundColor:'whitesmoke'}}>Semestre</th>
        ...
        <tbody>
          {list.agregados.map((elem, index) => (
            <tr key={index} onClick={() => handleRowClick(elem, index)} style={{cursor: "pointer"}}>
              <td>{elem.nivel}</td>
              <td>{elem.estado}</td>
              <td>{elem.titulo}</td>
