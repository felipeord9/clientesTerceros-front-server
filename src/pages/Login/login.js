...
export default function Login() {
  ...
  useEffect(() => {
    if (
      (isLogged && user.role === "cartera") ||
      (isLogged && user.role === "agencias")
    )
      navigate("/menu/principal/Clientes");
    if (
      (isLogged && user.role === "compras") ||
      (isLogged && user.role === "asistente agencia") ||
      (isLogged && user.role === "comprasnv")
    )
      navigate("/menu/principal/Proveedores");
    if (isLogged && user.role === "recursos humanos")
      navigate("/menu/principal/rrhh");
  ...
  return (
    ...
    >
      <div className="rounder-4">
        <div
          className="login-wrapper p-2 shadow-lg border-light rounded-4 border border-3 bg-gradient d-flexjustify-content-between "
        ...
        >
          ...
          <form onSubmit={handleLogin} className="">
            <div className="input_group m-3 ">
              ...
              />
              <label for="usuario" className="input_group_label">
                Usuario
              </label>
            </div>
            <div className="input_group m-3 d-flex flex-column">
              <input
                ...
                required
              />
              <label for="email" className="input_group_label">
                Contraseña
              </label>
              <span
                className="position-absolute"
                onClick={switchShown}
                style={{ right: 10, cursor: "pointer", fontSize: 25 }}
              >
                {shown ? <Bs.BsEye /> : <Bs.BsEyeSlash />}
              </span>
            </div>
            ...
            <center>
              <label>
                ...
                >
                  <strong>¿Olvidaste tu constraseña?</strong>
                </a>
              </label>
            </center>
          </form>
